...
namespace AutoGeneratePostman.Controllers
{
    ...
    public static class XmlDocumentationExtensions
    {
        ...
        {
            var moduleAssembly = member.Module.Assembly;
            AssemblyName assemblyName = moduleAssembly.GetName();
            var moduleAssemblyLocation = moduleAssembly.Location;
            var array = moduleAssemblyLocation.Split("\\");
            array = array.Take(array.Length - 1).ToArray();
            var @join = string.Join("\\", array);

            return GetXmlDocumentation(member, @join + "\\" + assemblyName.Name + ".xml");
        }
